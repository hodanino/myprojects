    ...
    public void run() {
        try {
            ...
            while (!shouldTerminate) {
                ...
                //need to implement methods and call for example RRQ to check if we have this file and open a file accordingly
                ...
                if(opCode == 1){
                    ...
                    String fileName = new String(message, 2, message.length - 3, StandardCharsets.UTF_8); // Replace with your desired file name
                    File file = new File(fileName);
                    if (!file.createNewFile()) {
                        System.out.println("File already exists");
                        continue;
                    }else listeningThread.fileNameRRQ = fileName;
                }
                else if(opCode == 2){//WRQ: if file exists here ERROR, else save all data
